...
def load_precursor_predictor(device = 'cpu'):
  ...
  class PrecursorTool(BaseTool):
    ...
    description = 'useful when you want to guess the precursors of a compound in chemical reaction. Each compound has mutiple possible groups of precursors which are stored in the return list. Each group of precursors can produce the target compound in a reaction independently. The groups are returned in descent order of possibility.'
